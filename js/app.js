...
const BG_SETTINGS_KEY = 'readerBackgroundSettings';
function getBackgroundSettings() {
    try {
        return JSON.parse(localStorage.getItem(BG_SETTINGS_KEY) || '{}');
    } catch { return {}; }
}
function setBackgroundSettings(settings) {
...
function applyBackgroundFromSettings() {
    ...
    readerContent.style.backgroundImage = s.image ? `url(${s.image})` : '';
    readerContent.style.backgroundSize = s.image ? 'cover' : '';
    readerContent.style.backgroundPosition = s.image ? 'center' : '';
    readerContent.style.backgroundColor = s.color || '';
}
    ...
    try {
        ...
        if (textItems && textItems.length > 0) {
            ...
            if (foundChapterStart) {
                ...
                if (currentDocument && currentDocument.name) {
                    canvasContainer.dataset.bookName = currentDocument.name;
                }
        ...
        for (let i = 0; i < pageElements.length; i++) {
            ...
            if (elementMiddle >= 0 && elementMiddle <= readerContent.clientHeight) {
                ...
                if (pageNumber !== currentPageNum) {
                    ...
                    element.classList.add('enter-anim');
                    element.classList.add('current');
                    setTimeout(() => element.classList.remove('enter-anim'), 400);
                }
                break;
            }
        }
...
function scrollToPage(pageNumber) {
    ...
    if (pageElement) {
        isScrolling = true;
        pageElement.classList.add('enter-anim');
        ...
        setTimeout(() => {
            isScrolling = false;
            pageElement.classList.remove('enter-anim');
        }, 500);
    }
}
